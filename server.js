...
const server=http.createServer(async(req,res)=>{
    ...
    if(req.method==='GET'&&parts.length===0){ res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Access-Control-Allow-Origin':'*'}); return res.end(fs.readFileSync(require('path').join(__dirname,'index.html'))); }
    ...
    socket.on('data',chunk=>{
        ...
        while(buf.length>=2){
            ...
            if(msg.type==='move'){
                ...
                // Если это ответ на req_pos (teleport:true) — рассылаем с флагом чтобы получатели не интерполировали
